...
def parse_sections_headers(
    ...
    if elf_header.e_shnum.value.value == 0 \
            or elf_header.e_shoff.value.value == 0 \
            or elf_header.e_shentsize.value.value == 0:
        return None,None
    ...
    for i in range(len(elf_sections)):
        elf_section = elf_sections[i]
        position = file.tell()
        ...
        #section name 可能是空的
        if elf_section.sh_type.information not in organize_sections:
            organize_sections[elf_section.sh_type.information] = {}

        if elf_section.name in organize_sections[elf_section.sh_type.information]  or len(elf_section.name) == 0:
            elf_section.name = elf_section.name + f"_${i}"
        update_dict(organize_sections[elf_section.sh_type.information], elf_section.name, elf_section)
...
def parse_layout_in_section(
        programs: Dict[str, List[Union[ProgramHeader32, ProgramHeader64]]],
        section_headers:List[Union[SectionHeader32, SectionHeader64]]):

    if section_headers == None:
        return None,None
    programs_PT_LOAD = programs['PT_LOAD']
    # 把要加载的拿出来,然后排序

    load_list_bysec = {}  # name , offset , size

    for section_item in section_headers:
        off = section_item.sh_addr.value
        if off.value == 0:
            continue
        (can_load, load_idx) = __check_isloaded(programs_PT_LOAD, off.value)
        if can_load == True:
            if load_idx not in load_list_bysec:
                load_list_bysec[load_idx] = []
            load_list_bysec[load_idx].append(
                {
    ...
    load_list_bysec = dict(sorted(load_list_bysec.items(), key=lambda x: x[0]))
    for load_idx, load_list_item in load_list_bysec.items():
        load_list_bysec[load_idx] = sorted(load_list_item, key=lambda x: x['offset'].value)
    #sorted_list = sorted(load_list, key=lambda x: list(x.values())[0])
    return load_list_bysec
...
def parse_elffile(
    ...
    # d_tag对应的数据在哪一个段, 类似于布局layout, 同时也解析了非PT_LOAD位于PT_LOAD段的哪里
    ...
    load_list_bysec = parse_layout_in_section(programs_headers_organize,section_headers)
